...
#define GUICOLOR_REGION_BG          (struct color_t){ 30, 30, 30 }
...
struct leafgui_region_t {
    ...
    bool enabled;
    bool active;
    int autopos_x; // Auto position when region was started.
    int autopos_y;
};
...
static struct GUI_T {
    ...
    struct leafgui_region_t region;
...
void leafgui_event_mouse_scroll(double yoffset) {
    ...
    *GUI.region.scroll_ptr += (int)yoffset * 8;
}
...
void leafgui_region_begin(char* label, int pos_x, int pos_y, int width, int height, int* scroll) {

    const bool auto_pos = (pos_x < 0 && pos_y < 0);
    if(auto_pos) {
        pos_x = GUI.prev_guielem_pos_x;
        pos_y = GUI.prev_guielem_pos_y + GUI.prev_guielem_height + GUI_AUTOPOS_PADDING_Y;
    }

    // Background.
    leaf_draw_rect(pos_x, pos_y, width, height, GUICOLOR_REGION_BG);

    GUI.region = (struct leafgui_region_t) {
        pos_x,
        pos_y,
        width,
        height
    ...
    GUI.region.autopos_x = GUI.prev_guielem_pos_x;
    GUI.region.autopos_y = GUI.prev_guielem_pos_y;
    GUI.region.scroll_ptr = scroll;
    GUI.region.enabled = true;
    GUI.region.active = false;

    GUI.prev_guielem_pos_x += 5;
    GUI.prev_guielem_pos_y += 5;
}

void leafgui_region_end() {
    GUI.region.enabled = false;
    GUI.prev_guielem_pos_x = GUI.region.autopos_x;
    GUI.prev_guielem_pos_y = GUI.region.autopos_y + GUI.region.height;
}

void leafgui_set_autopos_next(int pos_x, int pos_y) {
    GUI.prev_guielem_pos_x = pos_x;
    GUI.prev_guielem_pos_y = pos_y;
}

static bool leafgui_guielem_outside_current_region(int* pos_x, int* pos_y, int rect_w, int rect_h) {
    if(GUI.region.enabled) {
        *pos_y += *GUI.region.scroll_ptr;
        
        if(*pos_y + rect_h > GUI.region.pos_y + GUI.region.height) {
            return true;
        }
        else
        if(*pos_y < GUI.region.pos_y) {
            return true;
        }
...
(
    ...
    if(leafgui_guielem_outside_current_region(&pos_x, &pos_y, rect_w, rect_h)) {
        return false;
    }
