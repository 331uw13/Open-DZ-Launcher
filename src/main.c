...
void construct_tui(struct opendzl_ctx* ctx) {


    opendzltui_create_button(&ctx->tui, 3, 5, "test button", NULL);

}


void user_interaction_loop(struct opendzl_ctx* ctx) {
    while(ctx->running) {
        move(0, 0);
        clrtobot();
...
int main() {
    ...
    if(opendzl_get_server_info("192.168.1.140", 27016, &server_info)) {
